using Microsoft.EntityFrameworkCore;
using Microsoft.Extensions.DependencyInjection;
using DiscordBot.Database;

namespace DiscordBot
{
    public class Startup
    {
        public void ConfigureServices(IServiceCollection services)
        {
            //-------------------------------------------------------------------------------------------
            //Database Configuration
            //General

            //Database host
            var mariadb_host = Environment.GetEnvironmentVariable("MARIADB_HOST") ?? "mariadb";
            //Database Port 
            var mariadb_port = Environment.GetEnvironmentVariable("MARIADB_PORT") ?? "3306";
            //Database user 
            var mariaddb_user = Environment.GetEnvironmentVariable("MARIADB_USER") ?? "user";
            //Database password 
            var mariadb_password = Environment.GetEnvironmentVariable("MARIADB_PASSWORD") ?? "password";

            //-------------------------------------------------------------------------------------------
            //Database - Logs

            //Data base Name 
            var mariadb_databaselogs = Environment.GetEnvironmentVariable("MARIADB_DATABASELOGS") ?? "mariadb_databaselogs";

            // Connection String
            var connectionString = $"Server={mariadb_host};Port={mariadb_port};Database={mariadb_databaselogs};User={mariaddb_user};Password={mariadb_password};";

            // Server Version
            var serverVersion = ServerVersion.AutoDetect(connectionString);
            
            // DbContext registrieren
            services.AddDbContext<DataBaseLogs>(options =>
                options.UseMySql(connectionString, serverVersion)
            );
            //-------------------------------------------------------------------------------------------

            
            // Hier kommen später Discord Services rein
            // services.AddSingleton<DiscordSocketClient>();
        }
    }
}  